...
def train_trivial(data, g_net, d_net, name='TRIVIAL',
    ...
    for i in range(n_generators):
        # Common layers
        feat = g_net.former(zs, 'TRIVIAL_G', reuse=True if i > 0 else False)
    ...
    data_samples = ['data_samples_{:04d}_TRIVIAL.out']
    gen_samples = ['gen_samples_{:04d}_TRIVIAL.out']
    ...
    for it in range(int(n_iters)):
        ...
        _, loss_D = sess.run(
            [D_solver, D_loss],
            feed_dict={x0: batch_xs, z0: sampler(batch_size/n_generators, dim_z)}
        )

        _, loss_G = sess.run(
            [G_solver, G_loss],
            feed_dict={z0: sampler(batch_size/n_generators, dim_z)}
        )
        ...
        if it % PRNT_INTERVAL == 0:
            ...
            # Tensorboard
            cur_summary = sess.run(summaries, feed_dict={x0: batch_xs, z0: sampler(batch_size/n_generators, dim_z)})
