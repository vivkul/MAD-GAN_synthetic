...
class ToyNetInd(object):
    ...
    def former1(self, x, name=None, reuse=False):
        if not name:
            name = self.name
    ...
    def former2(self, fc1, name=None, reuse=False):
        if not name:
            name = self.name
        kwargs = dict(self.kwargs)  # Clone
    ...
    def latter(self, fc2, name=None, reuse=False, keep_same_output=True, new_output_dim=1, keep_same_act=True, new_act=tf.sigmoid):
        if not name:
            name = self.name
        kwargs = dict(self.kwargs)  # Clone
        ...
        with tf.variable_scope(name) as vs:
            ...
            if keep_same_act == False:
                kwargs['act'] = new_act

            out_dim = self.n_out
            if keep_same_output == False:
                out_dim = new_output_dim

            out = fc('fc3', fc2, [self.dim_h, out_dim], **kwargs)

        return out

    # Instantiate the network
    def __call__(self, x, name=None, reuse=False, **kwargs):
        feat = self.former1(x, name, reuse, **kwargs)
        feat = self.former2(feat, name, reuse, **kwargs)
