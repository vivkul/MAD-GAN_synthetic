...
class MoG1D:
    ...
    def plot(self, img_generator, gen_S = 'gen_samples.out', data_S = 'data_samples.out', fig_id=None, batch_size = 128):
        #fig, axs = plt.subplots(ncols=2)
        fig, axs = plt.subplots()
        sample_num = (1048576 / batch_size) * batch_size #65536
        samples = img_generator(sample_num) #(65536) #(batch_size * 8)  #TODO originally 1024, make it very high
        data_samples = self.next_batch(sample_num) #(65536) #(batch_size * 8)

        np.savetxt(gen_S, samples, delimiter=',')
        np.savetxt(data_S, data_samples, delimiter=',')
        sns.distplot(samples,ax=axs,bins=np.arange(-10, 130, 0.1),norm_hist=True,kde=False)
        sns.distplot(data_samples,ax=axs,bins=np.arange(-10, 130, 0.1),norm_hist=True,kde=False)
        return fig
...
def specs_MoG1D(size, lpf = 1, hpf = 1, std=0.1):
    ...
    std_ = [3,3,2,2,1] #TODO
    lines = []
    with open('datasets/specs1D_diffDist_1.txt') as f: #specs1D.txt
        for line in f:
            lines.append(line)
    ...
    for line in lines:
        i = float(line.rstrip())
        #j = float(line.rstrip().split()[1])
        #mog.add_mode(i, _std) #TODO _std
        mog.add_mode(i, std_[index])
        if index in lowProbMembr:
            for iterInd in range(mog.lowerProbFactor):
                mog.dataExtractor.append(index)
        else:
...
def specs_MoG1D_20mod(size, lpf = 1, hpf = 1, std=0.1):
    #uses specs.txt
    #assert(size % 2 == 1)
    ...
    std_ = 2 #TODO
    lines = []
    with open('datasets/specs1D_diffDist_20.txt') as f:
        for line in f:
            lines.append(line)
...
def specs_MoG1D_100mod(size, lpf = 1, hpf = 1, std=0.1):
    #uses specs.txt
    #assert(size % 2 == 1)
    ...
    std_ = 2 #TODO
    lines = []
    with open('datasets/specs1D_diffDist_100.txt') as f:
        for line in f:
            lines.append(line)
